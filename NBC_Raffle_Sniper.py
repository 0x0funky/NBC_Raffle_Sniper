...
def get_latest_nbc_tweet():
    ...
    for i in data["result"]:
        if i["raffle_event"]:
            try:
                mins, sec = time_difference(i["raffle_event"]["ExpiredAt"])
                expir_str = f"{mins} m, {sec} s"
            except:
                expir_str = "Finished"
            if i["raffle_event"]["Status"] == "running":
                if i["raffle_event"]["IsHolder"]:
                    # address = i["raffle_event"]["TwitterAddress"]
                    # amount_access = get_access_key_number(address)
                    amount_access = "Need Key"
                else:
                    amount_access = 0
                winner_avat = ""
                winner_username = "???"
                ...
                if i["raffle_event"]["IsHolder"]:
                    ...
                    amount_access = 0
                winner_avat = i["raffle_event"]["RaffleSpend"]["ToTwitter"]['TwitterAvatar']
                winner_username = i["raffle_event"]["RaffleSpend"]["ToTwitter"]['TwitterUsername']
                winner_address = i["raffle_event"]["RaffleSpend"]["ToTwitter"]['AddressChecked']
            if " #" in i["raffle_event"]["ERC721Name"]:
                ERC721name = i["raffle_event"]["ERC721Name"].split(" #")[0]
            else:
                ERC721name = i["raffle_event"]["ERC721Name"]
            entry = {
                "img_url": i["raffle_event"]["Twitter"]["TwitterAvatar"],
                "user_name": i["raffle_event"]["Twitter"]["TwitterUsername"],
                "user_address": i["raffle_event"]["TwitterAddress"],
                "expir_date": f"{expir_str}",
                "status": i["raffle_event"]["Status"],
                "is_holder": amount_access,
                "erc_name": ERC721name,
                "erc_address": i["raffle_event"]["ERC721Address"],
                "erc_ipfs": i["raffle_event"]['ERC721URI'],
...
def main_content():
    ...
    """, unsafe_allow_html=True)
            with col2:
                for data in new_data:
                    ...
                    st.markdown(f"""
    <div style='text-align: center; margin-bottom: 30px; margin-top: 10px; margin-left: -10px; white-space: nowrap;'>
    ...
    """, unsafe_allow_html=True)
            ...
            with col3:
                for data in new_data:
                    ERC_url = "https://pro.opensea.io/zh-TW/collection/"+data['erc_address']
                    st.markdown(f"""
    <div style='text-align: center; margin-bottom: 30px; margin-top: 10px; margin-left: -10px; white-space: nowrap;'>
        <a href="{ERC_url}" target="_blank">{data['erc_name']}</a>
    </div>
    """, unsafe_allow_html=True)
                    # st.markdown(f"<div style='text-align: center; margin-bottom: 30px; margin-top: 10px;white-space: nowrap;'>{data['erc_name']}</div>", unsafe_allow_html=True)
            with col4:
                for data in new_data:
...
title_text = "NewBitcoinCity Raffle Sniper"
...
st.write("Lucky Wheel Sniper (Eazy to find the lucky wheel): ", "https://nbc-luckywheel-sniper.streamlit.app/")
st.write('''
<style>
